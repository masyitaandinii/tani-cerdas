...
export function AdminClient() {
        ...
        async function fetchRecords() {
            try {
                ...
                if (res.ok) {
                    ...
                    setRecords(data.data || data); // handle API response structure gracefully
                } else {
    ...
    // Unauthenticated View: Render Login Form
    if (!session?.user) {
        return (
    ...
    // Authenticated View
    const dusunRecords = Array.isArray(records) ? records.filter((r) => {
    ...
    const kuartalTrendData = (() => {
        ...
        return kuartals.map((k) => {
            const kRecords = Array.isArray(records) ? records.filter(
